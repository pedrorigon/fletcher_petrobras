...
void RandomVelocityBoundary(int sx, int sy, int sz, int nx, int ny, int nz, int bord, int absorb, float *vpz, float *vsv) {

  int i, ix, iy, iz, distx, disty, distz, dist, ivelx, ively, ivelz, firstIn, bordLen;
  float bordDist, frac, rfac, maxP, maxS;
  ...
  for (iz=0; iz<sz; iz++) {
    for (iy=0; iy<sy; iy++) {
      for (ix=0; ix<sx; ix++) {
				...
				if ((iz>=firstIn && iz<=bordLen+nz) && (iy>=firstIn && iy<=bordLen+ny) && (ix>=firstIn && ix<=bordLen+nx)) {
				  continue;
				}
				// random speed inside absortion zone
				else if ((iz>=bord && iz<=2*bordLen+nz) && (iy>=bord && iy<=2*bordLen+ny) && (ix>=bord && ix<=2*bordLen+nx)) {
				  if (iz>bordLen+nz) {
				    distz=iz-bordLen-nz;
				    ivelz=bordLen+nz;
				  } else if (iz<firstIn) {
				  ...
				  vpz[i]=vpz[ind(ivelx,ively,ivelz)]*(1.0-bordDist)+maxP*rfac*bordDist;
				  vsv[i]=vsv[ind(ivelx,ively,ivelz)]*(1.0-bordDist)+maxS*rfac*bordDist;
				}
