...
__global__ void kernel_pDx_pDy(const int sx, const int sy, const int sz, 
			       const int iz, const int itz,
			       ...
			       float * restrict qp, const float * const restrict qc)
{
  ...
  // first derivative at x and y of a single grid point
...
__global__ void kernel_PropagateDer1Der1LM(const int sx, const int sy, const int sz, 
					   const int iz, 
                                           const int itM4, const int itM3, const int itM2, 
					   const int itM1, const int itz,  const int itP0, 
					   const int itP1, const int itP2, const int itP3, const int itP4,
...
#undef SAMPLE_PRE_LOOP
   ...
   int iz;
   for (iz=0; iz<2*bord; iz++){
        kernel_pDx_pDy<<<numBlocks, threadsPerBlock>>> (sx, sy, sz,
							iz, 0, 
							dxinv, dyinv, 
							strideX, strideY,
							dev_pDx, dev_qDx, 
...
#undef SAMPLE_LOOP_6
     ...
     kernel_PropagateDer1Der1LM<<<numBlocks, threadsPerBlock>>> (sx, sy, sz,
								 iz,
		                                                 itM4, itM3, itM2, itM1, itz,
                                                                 itP0, itP1, itP2, itP3, itP4,
								 dt, dyinv, dzinv,
