...
#include <openacc.h>
  ...
  cudaError_t err = cudaMallocManaged(&vpz, sx * sy * sz * sizeof(float)); // alocacao para usar unified memory.
  if (err != cudaSuccess)
  {
  ...
  cudaError_t err2 = cudaMallocManaged(&vsv, sx * sy * sz * sizeof(float));
