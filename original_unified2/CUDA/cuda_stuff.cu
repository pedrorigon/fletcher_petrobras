                     ...
                     float *restrict pp, float *restrict pc, float *restrict qp, float *restrict qc)
{
   ...
   CUDA_CALL(cudaMemPrefetchAsync(vpz, msize_vol, device, NULL));

   // CUDA_CALL(cudaMalloc(&dev_vsv, msize_vol));
   //(cudaMemcpy(dev_vsv, vsv, msize_vol, cudaMemcpyHostToDevice));
   CUDA_CALL(cudaMemPrefetchAsync(vsv, msize_vol, device, NULL));
...
void CUDA_Finalize(float *restrict vpz, float *restrict vsv)
{
