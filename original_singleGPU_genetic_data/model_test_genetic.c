 ...
 inicializarPopulacao(populacao, POPULATION_SIZE);
  ...
  for (int it=1; it<=st; it++) {
	...
	if (it % POPULATION_SIZE == 0 && it <= POPULATION_SIZE*3){
    	...
    	else if (it <= POPULATION_SIZE*3 && it % POPULATION_SIZE != 0)
        {
            printf("\nBsize_x: %d \n", populacao[it - geracao*POPULATION_SIZE - 1].thread_x);
            printf("Bsize_y: %d \n", populacao[it - geracao*POPULATION_SIZE - 1].thread_y);
            const double t0=wtime();
            DRIVER_Propagate(sx, sy, sz, bord, dx, dy, dz, dt, it, pp, pc, qp, qc, populacao[it - geracao*POPULATION_SIZE - 1].thread_x, populacao[it - geracao*POPULATION_SIZE - 1].thread_y);
            kernel_time=wtime()-t0;
            walltime+=kernel_time;
	    genetic_time+=kernel_time;
            res = (MEGA*(double)samplesPropagate)/kernel_time;
            populacao[it - geracao*POPULATION_SIZE - 1].fitness = 1 / kernel_time;
