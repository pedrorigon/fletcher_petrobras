...
grafico <- ggplot(dados, aes(x = reorder(factor(Tamanho), Tamanho), y = Resultado_Medio, fill = Aplicativo)) +
  ...
  scale_y_continuous(breaks = seq(0, max(dados$Resultado_Medio), by = 500), limits = c(0, max(dados$Resultado_Medio) * 1.1), expand = c(0, 0)) +
  theme_minimal() +
  theme(
    axis.text.x = element_text(size = 8, color = "black", hjust = 0.5, vjust = 0.5),  # Ajuste o tamanho da fonte do eixo x aqui
