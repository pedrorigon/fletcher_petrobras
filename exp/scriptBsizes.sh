...
for size in `seq 24 32 504`; do //identaar tudo daqui em diante
	...
	echo "Tamanho do Problema, Aplicativo, Bsize_X, Bsize_Y, Resultado Médio, Desvio Padrão, IC Inferior, IC Superior" > $output_file
	...
	for app in *.`hostname`.x; do
        	...
        	echo "$size, $app, $bsize_x, $bsize_y, $average_msamples, $stddev, $lower_bound, $upper_bound" >> "../$output_file"
