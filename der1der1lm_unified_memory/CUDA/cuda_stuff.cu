                     ...
                     float *restrict pp, float *restrict pc, float *restrict qp, float *restrict qc)
{
   ...
   const size_t msize_vol_tyle = sxsy * DIM_Z_TILE * sizeof(float);
   CUDA_CALL(cudaMallocManaged(&dev_pDx, msize_vol_tyle));
   CUDA_CALL(cudaMemset(dev_pDx, 0, msize_vol_tyle));
   CUDA_CALL(cudaMallocManaged(&dev_pDy, msize_vol_tyle));
   CUDA_CALL(cudaMemset(dev_pDy, 0, msize_vol_tyle));
   CUDA_CALL(cudaMallocManaged(&dev_qDx, msize_vol_tyle));
   CUDA_CALL(cudaMemset(dev_qDx, 0, msize_vol_tyle));
   CUDA_CALL(cudaMallocManaged(&dev_qDy, msize_vol_tyle));
   CUDA_CALL(cudaMemset(dev_qDy, 0, msize_vol_tyle));
